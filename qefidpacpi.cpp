...
QEFIDevicePath *qefi_parse_dp_acpi_hid(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_ACPI || dp->subtype != QEFIDevicePathACPISubType::ACPI_HID)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint32))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 hid = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    quint32 uid = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
...
QEFIDevicePath *qefi_parse_dp_acpi_hidex(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_ACPI || dp->subtype != QEFIDevicePathACPISubType::ACPI_HIDEX)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint32) + sizeof(quint32))
        return nullptr;
    ...
    quint32 uid = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    quint32 cid = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    // TODO: Parse strings, the string format is not clear
    return new QEFIDevicePathACPIHIDEX(hid, uid, cid, QString(), QString(), QString());
}
...
QEFIDevicePath *qefi_parse_dp_acpi_adr(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_ACPI || dp->subtype != QEFIDevicePathACPISubType::ACPI_ADR)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    for (int i = sizeof(struct qefi_device_path_header); i < length; i += sizeof(quint32)) {
        addresses << qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
        dp_inner_pointer += sizeof(quint32);
    }
...
QByteArray qefi_format_dp_acpi_hid(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_ACPI || dp->subType() != QEFIDevicePathACPISubType::ACPI_HID)
        return QByteArray();
    QEFIDevicePathACPIHID *dp_instance = dynamic_cast<QEFIDevicePathACPIHID *>(dp);
    ...
    // Append the fields
    quint32 hid = qToLittleEndian<quint32>(dp_instance->hid());
    buffer.append((const char *)&hid, sizeof(quint32));
    quint32 uid = qToLittleEndian<quint32>(dp_instance->uid());
...
QByteArray qefi_format_dp_acpi_hidex(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_ACPI || dp->subType() != QEFIDevicePathACPISubType::ACPI_HIDEX)
        return QByteArray();
    QEFIDevicePathACPIHIDEX *dp_instance = dynamic_cast<QEFIDevicePathACPIHIDEX *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint32 uid = qToLittleEndian<quint32>(dp_instance->uid());
    buffer.append((const char *)&uid, sizeof(quint32));
    quint32 cid = qToLittleEndian<quint32>(dp_instance->cid());
...
QByteArray qefi_format_dp_acpi_adr(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_ACPI || dp->subType() != QEFIDevicePathACPISubType::ACPI_ADR)
        return QByteArray();
    QEFIDevicePathACPIADR *dp_instance = dynamic_cast<QEFIDevicePathACPIADR *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QEFIDevicePathACPIHID::QEFIDevicePathACPIHID(quint32 hid, quint32 uid)
    : QEFIDevicePathACPI((quint8)QEFIDevicePathACPISubType::ACPI_HID)
    , m_hid(hid)
    , m_uid(uid)
{
}

QEFIDevicePathACPIHIDEX::QEFIDevicePathACPIHIDEX(quint32 hid, quint32 uid, quint32 cid, QString hidString, QString uidString, QString cidString)
    : QEFIDevicePathACPI((quint8)QEFIDevicePathACPISubType::ACPI_HIDEX)
    , m_hid(hid)
    , m_uid(uid)
    , m_cid(cid)
    , m_hidString(hidString)
