...
// Message parsing
QEFIDevicePath *qefi_parse_dp_message_atapi(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_ATAPI)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) + sizeof(quint8) + sizeof(quint16))
        return nullptr;
    ...
    dp_inner_pointer += sizeof(quint8);
    quint16 lun = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    return new QEFIDevicePathMessageATAPI(primary, slave, lun);
}

QEFIDevicePath *qefi_parse_dp_message_scsi(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_SCSI)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint16) + sizeof(quint16))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint16 target = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 lun = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    return new QEFIDevicePathMessageSCSI(target, lun);
}

QEFIDevicePath *qefi_parse_dp_message_fibre_chan(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_FibreChan)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    quint64 wwn = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint64);
    quint64 lun = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    return new QEFIDevicePathMessageFibreChan(reserved, wwn, lun);
}

QEFIDevicePath *qefi_parse_dp_message_1394(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_1394)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint64))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 reversed = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    quint64 guid = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    return new QEFIDevicePathMessage1394(reversed, guid);
}

QEFIDevicePath *qefi_parse_dp_message_usb(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_USB)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_i2o(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_I2O)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 target = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    return new QEFIDevicePathMessageI2O(target);
}

QEFIDevicePath *qefi_parse_dp_message_infiniband(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_InfiniBand)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint64) + sizeof(quint64) + sizeof(quint64) + sizeof(quint64) + sizeof(quint64))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 resourceFlags = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    quint64 portGID1 = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint64);
    quint64 portGID2 = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint64);
    quint64 sharedField = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint64);
    quint64 targetPortID = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint64);
    quint64 deviceID = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    return new QEFIDevicePathMessageInfiniBand(resourceFlags, portGID1, portGID2, sharedField, targetPortID, deviceID);
}

QEFIDevicePath *qefi_parse_dp_message_vendor(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_Vendor)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_mac_addr(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_MACAddr)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 32 + sizeof(quint8))
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_ipv4(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_IPv4)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 4 + sizeof(quint8) * 4 + sizeof(quint16) + sizeof(quint16) + sizeof(quint16) + sizeof(quint8)
    ...
    dp_inner_pointer += sizeof(quint8) * 4;
    quint16 localPort = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 remotePort = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 protocol = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    ...
    return new QEFIDevicePathMessageIPv4Addr(localIPv4Addr, remoteIPv4Addr, localPort, remotePort, protocol, staticIPAddr, gateway, netmask);
}

QEFIDevicePath *qefi_parse_dp_message_ipv6(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_IPv6)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 16 + sizeof(quint8) * 16 + sizeof(quint16) + sizeof(quint16) + sizeof(quint16)
    ...
    return new QEFIDevicePathMessageIPv6Addr(localIPv6Addr, remoteIPv6Addr, localPort, remotePort, protocol, ipAddrOrigin, prefixLength, gatewayIPv6Addr);
}

QEFIDevicePath *qefi_parse_dp_message_uart(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_UART)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint64) + sizeof(quint8) + sizeof(quint8) + sizeof(quint8))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 reserved = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    quint64 baudRate = qFromLittleEndian<quint64>(*((quint64 *)dp_inner_pointer));
    ;
    dp_inner_pointer += sizeof(quint64);
    ...
    return new QEFIDevicePathMessageUART(reserved, baudRate, dataBits, parity, stopBits);
}

QEFIDevicePath *qefi_parse_dp_message_usb_class(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_USBClass)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint16) + sizeof(quint16) + sizeof(quint8) + sizeof(quint8) + sizeof(quint8))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint16 vendorId = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 productId = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    ...
    dp_inner_pointer += sizeof(quint8);
    return new QEFIDevicePathMessageUSBClass(vendorId, productId, deviceClass, deviceSubclass, deviceProtocol);
}

QEFIDevicePath *qefi_parse_dp_message_usb_wwid(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_USBWWID)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_lun(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_LUN)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_sata(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_SATA)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint16) + sizeof(quint16) + sizeof(quint8))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint16 hbaPort = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 portMultiplierPort = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint8 lun = *dp_inner_pointer;
    return new QEFIDevicePathMessageSATA(hbaPort, portMultiplierPort, lun);
}

QEFIDevicePath *qefi_parse_dp_message_iscsi(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_ISCSI)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint16) + sizeof(quint16) + sizeof(quint8) * 16 + sizeof(quint16))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint16 protocol = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint16 options = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    quint8 *lun = dp_inner_pointer;
    dp_inner_pointer += sizeof(quint8) * 8;
    quint16 tpgt = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint16);
    QString targetName(QByteArray((const char *)dp_inner_pointer, length - (dp_inner_pointer - (quint8 *)dp)));
    return new QEFIDevicePathMessageISCSI(protocol, options, lun, tpgt, targetName);
}

QEFIDevicePath *qefi_parse_dp_message_vlan(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_VLAN)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint16))
        return nullptr;

    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint16 vlanID = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    return new QEFIDevicePathMessageVLAN(vlanID);
}

QEFIDevicePath *qefi_parse_dp_message_fibre_chan_ex(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_FibreChanEx)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint32) + sizeof(quint8) * 8 + sizeof(quint8) * 8)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_sas_ex(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_SASEX)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 8 + sizeof(quint8) * 8 + sizeof(quint8) + sizeof(quint8) + sizeof(quint16))
        return nullptr;
    ...
    dp_inner_pointer += sizeof(quint8);
    quint16 rtp = qFromLittleEndian<quint16>(*((quint16 *)dp_inner_pointer));
    return new QEFIDevicePathMessageSASEx(sasAddress, lun, deviceTopologyInfo, driveBayID, rtp);
}

QEFIDevicePath *qefi_parse_dp_message_nvme(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_NVME)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    quint32 nid = qFromLittleEndian<quint32>(*((quint32 *)dp_inner_pointer));
    dp_inner_pointer += sizeof(quint32);
    return new QEFIDevicePathMessageNVME(nid, dp_inner_pointer);
}

QEFIDevicePath *qefi_parse_dp_message_uri(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_URI)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    quint8 *dp_inner_pointer = ((quint8 *)dp) + sizeof(struct qefi_device_path_header);
    QUrl uri(QString(QByteArray((const char *)dp_inner_pointer, length - (dp_inner_pointer - (quint8 *)dp))));
    return new QEFIDevicePathMessageURI(uri);
}

QEFIDevicePath *qefi_parse_dp_message_ufs(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_UFS)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_sd(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_SD)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_bt(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_BT)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 6)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_wifi(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_WiFi)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    QString ssid(QByteArray((const char *)dp_inner_pointer, ssid_len < 32 ? ssid_len : 32));
    return new QEFIDevicePathMessageWiFi(ssid);
}

QEFIDevicePath *qefi_parse_dp_message_emmc(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_EMMC)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_btle(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_BTLE)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 6 + sizeof(quint8))
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_dns(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_DNS)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
...
QEFIDevicePath *qefi_parse_dp_message_nvdimm(struct qefi_device_path_header *dp, int dp_size)
{
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE)
        return nullptr;
    if (dp->type != QEFIDevicePathType::DP_Message || dp->subtype != QEFIDevicePathMessageSubType::MSG_NVDIMM)
        return nullptr;
    int length = qefi_dp_length(dp);
    if (length != dp_size || length <= 0)
        return nullptr;
    ...
    // Check size
    if (dp_size < QEFI_DEVICE_PATH_HEADER_SIZE + sizeof(quint8) * 8)
        return nullptr;
...
QByteArray qefi_format_dp_message_atapi(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_ATAPI)
        return QByteArray();
    QEFIDevicePathMessageATAPI *dp_instance = dynamic_cast<QEFIDevicePathMessageATAPI *>(dp);
    ...
    quint16 lun = qToLittleEndian<quint16>(dp_instance->lun());
...
QByteArray qefi_format_dp_message_scsi(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_SCSI)
        return QByteArray();
    QEFIDevicePathMessageSCSI *dp_instance = dynamic_cast<QEFIDevicePathMessageSCSI *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint16 target = qToLittleEndian<quint16>(dp_instance->target());
...
QByteArray qefi_format_dp_message_fibre_chan(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_FibreChan)
        return QByteArray();
    QEFIDevicePathMessageFibreChan *dp_instance = dynamic_cast<QEFIDevicePathMessageFibreChan *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint64 wwn = qToLittleEndian<quint64>(dp_instance->wwn());
    buffer.append((const char *)&wwn, sizeof(quint64));
    quint64 lun = qToLittleEndian<quint64>(dp_instance->lun());
...
QByteArray qefi_format_dp_message_1394(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_1394)
        return QByteArray();
    QEFIDevicePathMessage1394 *dp_instance = dynamic_cast<QEFIDevicePathMessage1394 *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint32 reserved = qToLittleEndian<quint32>(dp_instance->reserved());
    buffer.append((const char *)&reserved, sizeof(quint32));
    quint64 guid = qToLittleEndian<quint64>(dp_instance->guid());
...
QByteArray qefi_format_dp_message_usb(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_USB)
        return QByteArray();
    QEFIDevicePathMessageUSB *dp_instance = dynamic_cast<QEFIDevicePathMessageUSB *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_i2o(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_I2O)
        return QByteArray();
    QEFIDevicePathMessageI2O *dp_instance = dynamic_cast<QEFIDevicePathMessageI2O *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint32 target = qToLittleEndian<quint32>(dp_instance->target());
...
QByteArray qefi_format_dp_message_infiniband(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_InfiniBand)
        return QByteArray();
    QEFIDevicePathMessageInfiniBand *dp_instance = dynamic_cast<QEFIDevicePathMessageInfiniBand *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint32 resourceFlags = qToLittleEndian<quint32>(dp_instance->resourceFlags());
    buffer.append((const char *)&resourceFlags, sizeof(quint32));
    quint64 portGID1 = qToLittleEndian<quint64>(dp_instance->portGID1());
    buffer.append((const char *)&portGID1, sizeof(quint64));
    quint64 portGID2 = qToLittleEndian<quint64>(dp_instance->portGID2());
    buffer.append((const char *)&portGID2, sizeof(quint64));
    quint64 sharedField = qToLittleEndian<quint64>(dp_instance->serviceID());
    buffer.append((const char *)&sharedField, sizeof(quint64));
    quint64 targetPortID = qToLittleEndian<quint64>(dp_instance->targetPortID());
    buffer.append((const char *)&targetPortID, sizeof(quint64));
    quint64 deviceID = qToLittleEndian<quint64>(dp_instance->deviceID());
...
QByteArray qefi_format_dp_message_vendor(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_Vendor)
        return QByteArray();
    QEFIDevicePathMessageVendor *dp_instance = dynamic_cast<QEFIDevicePathMessageVendor *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_mac_addr(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_MACAddr)
        return QByteArray();
    QEFIDevicePathMessageMACAddr *dp_instance = dynamic_cast<QEFIDevicePathMessageMACAddr *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    QEFIDevicePathMessageMACAddress address = dp_instance->macAddress();
    for (int i = 0; i < 32; i++) {
        // Append Mac Addr
...
QByteArray qefi_format_dp_message_ipv4(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_IPv4)
        return QByteArray();
    QEFIDevicePathMessageIPv4Addr *dp_instance = dynamic_cast<QEFIDevicePathMessageIPv4Addr *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint16 localPort = qToLittleEndian<quint16>(dp_instance->localPort());
    buffer.append((const char *)&localPort, sizeof(quint16));
    quint16 remotePort = qToLittleEndian<quint16>(dp_instance->remotePort());
    buffer.append((const char *)&remotePort, sizeof(quint16));
    quint16 protocol = qToLittleEndian<quint16>(dp_instance->protocol());
...
QByteArray qefi_format_dp_message_ipv6(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_IPv6)
        return QByteArray();
    QEFIDevicePathMessageIPv6Addr *dp_instance = dynamic_cast<QEFIDevicePathMessageIPv6Addr *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_uart(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_UART)
        return QByteArray();
    QEFIDevicePathMessageUART *dp_instance = dynamic_cast<QEFIDevicePathMessageUART *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint32 reserved = qToLittleEndian<quint32>(dp_instance->reserved());
    buffer.append((const char *)&reserved, sizeof(quint32));
    quint64 baudRate = qToLittleEndian<quint64>(dp_instance->baudRate());
...
QByteArray qefi_format_dp_message_usb_class(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_USBClass)
        return QByteArray();
    QEFIDevicePathMessageUSBClass *dp_instance = dynamic_cast<QEFIDevicePathMessageUSBClass *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint16 vendorId = qToLittleEndian<quint16>(dp_instance->vendorId());
    buffer.append((const char *)&vendorId, sizeof(quint16));
    quint16 productId = qToLittleEndian<quint16>(dp_instance->productId());
...
QByteArray qefi_format_dp_message_usb_wwid(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_USBWWID)
        return QByteArray();
    QEFIDevicePathMessageUSBWWID *dp_instance = dynamic_cast<QEFIDevicePathMessageUSBWWID *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_lun(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_LUN)
        return QByteArray();
    QEFIDevicePathMessageLUN *dp_instance = dynamic_cast<QEFIDevicePathMessageLUN *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_sata(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_SATA)
        return QByteArray();
    QEFIDevicePathMessageSATA *dp_instance = dynamic_cast<QEFIDevicePathMessageSATA *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint16 hbaPort = qToLittleEndian<quint16>(dp_instance->hbaPort());
    buffer.append((const char *)&hbaPort, sizeof(quint16));
    quint16 portMultiplierPort = qToLittleEndian<quint16>(dp_instance->portMultiplierPort());
...
QByteArray qefi_format_dp_message_iscsi(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_ISCSI)
        return QByteArray();
    QEFIDevicePathMessageISCSI *dp_instance = dynamic_cast<QEFIDevicePathMessageISCSI *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    quint16 protocol = qToLittleEndian<quint16>(dp_instance->protocol());
    buffer.append((const char *)&protocol, sizeof(quint16));
    quint16 options = qToLittleEndian<quint16>(dp_instance->options());
    ...
    quint16 tpgt = qToLittleEndian<quint16>(dp_instance->tpgt());
...
QByteArray qefi_format_dp_message_vlan(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_VLAN)
        return QByteArray();
    QEFIDevicePathMessageVLAN *dp_instance = dynamic_cast<QEFIDevicePathMessageVLAN *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint16 vlanID = qToLittleEndian<quint16>(dp_instance->vlanID());
...
QByteArray qefi_format_dp_message_fibre_chan_ex(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_FibreChanEx)
        return QByteArray();
    QEFIDevicePathMessageFibreChanEx *dp_instance = dynamic_cast<QEFIDevicePathMessageFibreChanEx *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_sas_ex(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_SASEX)
        return QByteArray();
    QEFIDevicePathMessageSASEx *dp_instance = dynamic_cast<QEFIDevicePathMessageSASEx *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    QEFIDevicePathMessageSASAddress address = dp_instance->sasAddress();
    for (int i = 0; i < 8; i++) {
        // Append sasAddress
        buffer.append((const char)address.address[i]);
    }
    ...
    quint16 rtp = qToLittleEndian<quint16>(dp_instance->rtp());
...
QByteArray qefi_format_dp_message_nvme(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_NVME)
        return QByteArray();
    QEFIDevicePathMessageNVME *dp_instance = dynamic_cast<QEFIDevicePathMessageNVME *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    quint32 namespaceID = qToLittleEndian<quint32>(dp_instance->namespaceID());
...
QByteArray qefi_format_dp_message_uri(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_URI)
        return QByteArray();
    QEFIDevicePathMessageURI *dp_instance = dynamic_cast<QEFIDevicePathMessageURI *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_ufs(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_UFS)
        return QByteArray();
    QEFIDevicePathMessageUFS *dp_instance = dynamic_cast<QEFIDevicePathMessageUFS *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_sd(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_SD)
        return QByteArray();
    QEFIDevicePathMessageSD *dp_instance = dynamic_cast<QEFIDevicePathMessageSD *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_bt(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_BT)
        return QByteArray();
    QEFIDevicePathMessageBT *dp_instance = dynamic_cast<QEFIDevicePathMessageBT *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
    ...
    buffer.append((char)0);
    // Append the fields
    QEFIDevicePathMessageBTAddress addr = dp_instance->address();
    for (int i = 0; i < 6; i++) {
        // Get the BT addr
...
QByteArray qefi_format_dp_message_wifi(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_WiFi)
        return QByteArray();
    QEFIDevicePathMessageWiFi *dp_instance = dynamic_cast<QEFIDevicePathMessageWiFi *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_emmc(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_EMMC)
        return QByteArray();
    QEFIDevicePathMessageEMMC *dp_instance = dynamic_cast<QEFIDevicePathMessageEMMC *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_btle(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_BTLE)
        return QByteArray();
    QEFIDevicePathMessageBTLE *dp_instance = dynamic_cast<QEFIDevicePathMessageBTLE *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_dns(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_DNS)
        return QByteArray();
    QEFIDevicePathMessageDNS *dp_instance = dynamic_cast<QEFIDevicePathMessageDNS *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QByteArray qefi_format_dp_message_nvdimm(QEFIDevicePath *dp)
{
    if (dp->type() != QEFIDevicePathType::DP_Message || dp->subType() != QEFIDevicePathMessageSubType::MSG_NVDIMM)
        return QByteArray();
    QEFIDevicePathMessageNVDIMM *dp_instance = dynamic_cast<QEFIDevicePathMessageNVDIMM *>(dp);
    if (dp_instance == nullptr)
        return QByteArray();
...
QEFIDevicePathMessageATAPI::QEFIDevicePathMessageATAPI(quint8 primary, quint8 slave, quint16 lun)
...
QEFIDevicePathMessageSCSI::QEFIDevicePathMessageSCSI(quint16 target, quint16 lun)
...
QEFIDevicePathMessageFibreChan::QEFIDevicePathMessageFibreChan(quint32 reserved, quint64 wwn, quint64 lun)
...
QEFIDevicePathMessage1394::QEFIDevicePathMessage1394(quint32 reserved, quint64 guid)
...
QEFIDevicePathMessageUSB::QEFIDevicePathMessageUSB(quint8 parentPort, quint8 inter)
...
QEFIDevicePathMessageInfiniBand::QEFIDevicePathMessageInfiniBand(quint32 resourceFlags,
                                                                 quint64 portGID1,
                                                                 quint64 portGID2,
                                                                 quint64 sharedField,
...
QEFIDevicePathMessageVendor::QEFIDevicePathMessageVendor(QUuid vendorGuid, QByteArray vendorData)
...
QEFIDevicePathMessageMACAddr::QEFIDevicePathMessageMACAddr(quint8 *macAddress, quint8 interfaceType)
...
QEFIDevicePathMessageIPv4Addr::QEFIDevicePathMessageIPv4Addr(quint8 *localIPv4Addr,
                                                             quint8 *remoteIPv4Addr,
                                                             quint16 localPort,
                                                             quint16 remotePort,
                                                             quint16 protocol,
                                                             quint8 staticIPAddr,
    ...
    , m_protocol(protocol)
    , m_staticIPAddress(staticIPAddr)
{
    for (int i = 0; i < 4; i++) {
...
QEFIDevicePathMessageIPv6Addr::QEFIDevicePathMessageIPv6Addr(quint8 *localIPv6Addr,
                                                             quint8 *remoteIPv6Addr,
                                                             quint16 localPort,
                                                             quint16 remotePort,
                                                             quint16 protocol,
                                                             quint8 ipAddrOrigin,
                                                             quint8 prefixLength,
                                                             quint8 gatewayIPv6Addr)
    : QEFIDevicePathMessage(MSG_IPv6)
    , m_localPort(localPort)
    , m_remotePort(remotePort)
    , m_protocol(protocol)
    , m_ipAddressOrigin(ipAddrOrigin)
...
QEFIDevicePathMessageUART::QEFIDevicePathMessageUART(quint32 reserved, quint64 baudRate, quint8 dataBits, quint8 parity, quint8 stopBits)
...
QEFIDevicePathMessageUSBClass::QEFIDevicePathMessageUSBClass(quint16 vendorId,
                                                             quint16 productId,
                                                             quint8 deviceClass,
    ...
    , m_deviceSubclass(deviceSubclass)
...
QEFIDevicePathMessageUSBWWID::QEFIDevicePathMessageUSBWWID(quint16 vendorId, quint16 productId, quint16 *sn)
...
QEFIDevicePathMessageSATA::QEFIDevicePathMessageSATA(quint16 hbaPort, quint16 portMultiplierPort, quint8 lun)
    : QEFIDevicePathMessage(MSG_SATA)
    , m_hbaPort(hbaPort)
    , m_portMultiplierPort(portMultiplierPort)
    , m_lun(lun)
{
}
...
QEFIDevicePathMessageISCSI::QEFIDevicePathMessageISCSI(quint16 protocol, quint16 options, quint8 *lun, quint16 tpgt, QString targetName)
...
QEFIDevicePathMessageVLAN::QEFIDevicePathMessageVLAN(quint16 vlanID)
    : QEFIDevicePathMessage(MSG_VLAN)
    , m_vlanID(vlanID)
{
}

QEFIDevicePathMessageFibreChanEx::QEFIDevicePathMessageFibreChanEx(quint32 reserved, quint8 *wwn, quint8 *lun)
    : QEFIDevicePathMessage(MSG_FibreChanEx)
    , m_reserved(reserved)
{
    for (int i = 0; i < 8; i++) {
        m_wwn.data[i] = wwn[i];
...
QEFIDevicePathMessageSASEx::QEFIDevicePathMessageSASEx(quint8 *sasAddress, quint8 *lun, quint8 deviceTopologyInfo, quint8 driveBayID, quint16 rtp)
    : QEFIDevicePathMessage(MSG_SASEX)
    , m_deviceTopologyInfo(deviceTopologyInfo)
    , m_driveBayID(driveBayID)
    , m_rtp(rtp)
{
    for (int i = 0; i < 8; i++) {
...
QEFIDevicePathMessageNVME::QEFIDevicePathMessageNVME(quint32 nid, quint8 *ieeeEui64)
    : QEFIDevicePathMessage(MSG_NVME)
    , m_namespaceID(nid)
{
...
QEFIDevicePathMessageUFS::QEFIDevicePathMessageUFS(quint8 targetID, quint8 lun)
...
QEFIDevicePathMessageWiFi::QEFIDevicePathMessageWiFi(QString ssid)
    : QEFIDevicePathMessage(MSG_WiFi)
    , m_ssid(ssid)
{
}
...
QEFIDevicePathMessageEMMC::QEFIDevicePathMessageEMMC(quint8 slot)
    : QEFIDevicePathMessage(MSG_EMMC)
    , m_slotNumber(slot)
{
}
...
QEFIDevicePathMessageBTLE::QEFIDevicePathMessageBTLE(quint8 *address, quint8 addressType)
...
QEFIDevicePathMessageNVDIMM::QEFIDevicePathMessageNVDIMM(QUuid uuid)
    : QEFIDevicePathMessage(MSG_NVDIMM)
    , m_uuid(uuid)
{
}
